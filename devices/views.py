...
def device_full_setup(request, pk):
    """Одной кнопкой: сменить пароль + info2mqtt + VNC (запускается в фоне
    через Celery, чтобы не блокировать запрос SSH-операциями)."""
    device = get_object_or_404(Device, pk=pk)
    if request.method == 'POST':
        if not device.vpn_ip or device.vpn_ip in ('0', 'N/A'):
            messages.error(request, f'{device.hostname}: нет VPN IP')
            return redirect('device_detail_page', pk=pk)
        admin_tg = getattr(request.user.profile, 'telegram_id', None)
        from devices.tasks import full_setup_device
        try:
            full_setup_device.delay(device.pk, admin_tg)
            messages.info(
                request,
                f'{device.hostname}: Полная настройка запущена в фоне.'
                f"{' Результат придёт сообщением в Telegram.' if admin_tg else ''}")
        except Exception as e:  # noqa: BLE001
            logger.error('full_setup: ошибка постановки задачи: %s', e)
            messages.error(request, f'{device.hostname}: не удалось запустить фоновую задачу')
    return redirect('device_detail_page', pk=pk)
